...
const FieldsModal = ({ event, close }) => {
  ...
  return (
    <motion.div
      onClick={handleOutside} // ✅ FIXED (was onMouseDown)
...
"
                >
                  ...
                  <td className="py-4 sm:py-5">
                    <span
                      className={`inline-flex items-center justify-center px-3 py-1 text-xs font-medium rounded-full min-w-[60px]
${
  f.required ? "bg-green-500/20 text-green-400" : "bg-gray-500/20 text-gray-400"
}`}
                    >
                      {f.required ? "Yes" : "No"}
                    </span>
      ...
      {confirmDelete && (
        <motion.div
          ...
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          onClick={() => setConfirmDelete(false)} // ✅ FIXED
        >
          <motion.div
